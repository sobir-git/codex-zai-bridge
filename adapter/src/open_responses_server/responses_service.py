...
def build_message_item(message_id: str, text: str, status: str) -> Dict[str, Any]:
    return {
        "id": message_id,
        "type": "message",
        "role": "assistant",
...
async def process_chat_completions_stream(response, chat_request=None):
    ...
    async def ensure_message_started():
        ...
        response_obj.output = [build_message_item(message_id, "", "in_progress")]
        yield f"data: {json.dumps({'type': 'response.output_item.added', 'output_index': 0, 'item': {'id': message_id, 'type': 'message', 'role': 'assistant', 'status': 'in_progress', 'content': []}})}\n\n"
        yield f"data: {json.dumps({'type': 'response.content_part.added', 'item_id': message_id, 'output_index': 0, 'content_index': 0, 'part': {'type': 'output_text', 'text': '', 'annotations': [], 'logprobs': None}})}\n\n"
                ...
                if "choices" in data and data["choices"]:
                    ...
                    if "delta" in choice:
                        ...
                        # Handle content (text)
                            ...
                            response_obj.output = [build_message_item(message_id, output_text_content, "in_progress")]
                    ...
                    if "finish_reason" in choice and choice["finish_reason"] is not None:
                        ...
                        if choice["finish_reason"] == "stop":
                            ...
                            response_obj.status = "completed"
                            final_text = output_text_content or "(No update)"
                            response_obj.output = [build_message_item(message_id, final_text, "completed")]
                            yield f"data: {json.dumps({'type': 'response.output_text.done', 'item_id': message_id, 'output_index': 0, 'content_index': 0, 'text': final_text})}\n\n"
                            yield f"data: {json.dumps({'type': 'response.content_part.done', 'item_id': message_id, 'output_index': 0, 'content_index': 0, 'part': {'type': 'output_text', 'text': final_text, 'annotations': [], 'logprobs': None}})}\n\n"
                            yield f"data: {json.dumps({'type': 'response.output_item.done', 'output_index': 0, 'item': response_obj.output[0]})}\n\n"
                            completed_event = ResponseCompleted(
                                type="response.completed",
                                response=response_obj
                            )
